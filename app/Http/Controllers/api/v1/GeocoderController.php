...
//                        'debug' => true,
        ...
        $data = json_decode($response->body(), true)['result'];
        $data = json_decode($response->body())->result;
...
//!!!!!            $item = (object)$item;
            ...
            $name = '';
            if (isset($item->localityType->name))
                $name .= $item->localityType->name . ' ';
            $name .= $item->name;
            $resultItem['name'] = $name;
            $resultItem['cityId'] = $item->cityId;
//?            $resultItem['postCode'] = $item->postCode;

            $result[] = $resultItem;
        }

        return ["success" => true, "language" => $request->input('lang'), 'result' => $result];
